...
from . import config
from . import questblueAPI as quest
...
def __singTrunkEndMon():
    quest.logInfo("---STARTING END MONTH SCRIPT (Single Trunk) ---")
    global outPath_endMon
    global start_date_endMon
    global end_date_endMon
    global endMon_size
    global trunk
    local_trunk = trunk.get()
    quest.init(username.get(), password.get(), key.get())
    quest.startCSV('endMon', str(trunk.get()))
    progressBar_endMon.set(0)
    local_month_start = endMon_month_start.get()
    local_year_start = endMon_year_start.get()
    local_day_start = endMon_day_start.get()
    ...
    quest.runDid(local_trunk, start_date_endMon, end_date_endMon)
...
def __thread_sing_trunk():
    thread = threading.Thread(target=__singWorker)
    thread.start()
    thread.join()
    return
...
def __worker_endMon():
    ...
    while not q_endMon.empty():
        trunk = q_endMon.get()
        print(f"Got trunk --- {trunk}")
        quest.logInfo(f"Thread  {trunk}: Starting")
        done = endMon_size - q_endMon.qsize()
        progressBar_endMon.set(float(done)/float(endMon_size))
        quest.getMonthReport_Trunk(trunk, start_date_endMon, end_date_endMon)
        
        quest.logInfo(f"Thread {trunk}: Ending")
...
def __thread_endMon():
    ...
    for t in range(max_threads):
        thread = threading.Thread(target=__worker_endMon)
...
def __thread_TF():
    ...
    for t in range(max_threads):
        thread = threading.Thread(target=__worker_TF)
        thread_list.append(thread)
...
def Login(root):
    ...
    try:
        with open('./temp/login.tmp', 'r+') as rememberFile:
            fileExists = True
    ...
    login_button = customtkinter.CTkButton(master=page, text="Login", command=__check_Login_filled)
    remember_me_box = customtkinter.CTkCheckBox(master=page, text="Remember Me", variable=checkRemember)
    version_label = customtkinter.CTkLabel(master=root, text=version, font=("Roboto", 12))
...
def __Menu(root):
    ...
    img = customtkinter.CTkImage(Image.open(__find_by_relative_path("assets" + os.sep + "HD-TEC-Logo.png")), size=(100,100))
    ...
    tf_button = customtkinter.CTkButton(master=page, text="Select", font=("Roboto", 12), command=__change_tf)

    endMon_label = customtkinter.CTkLabel(master=page, text="End Of Month", font=("Roboto", 14))
    endMon_button = customtkinter.CTkButton(master=page, text="Select", font=("Roboto", 12), command=__change_endMon)

    back_button = customtkinter.CTkButton(master=page, text="Back", font=("Roboto", 12), command=__change_login)
    version_label = customtkinter.CTkLabel(master=root, text=version, font=("Roboto", 12))
    ...
    quest.logInfo("Displayed __Menu")


def __EndMonPage(root):
    ...
    trunk = customtkinter.StringVar(root)
    ...
    trunk_label = customtkinter.CTkLabel(master=page, text="Trunk", font=("Roboto", 18))
    trunk_sel = customtkinter.CTkEntry(master=page, textvariable=trunk)
    ...
    back_button = customtkinter.CTkButton(master=page, text="Back", font=("Roboto", 12), command=__change_Menu)
    start_button = customtkinter.CTkButton(master=page, text="Start", font=("Roboto", 12), command=__startEndMonSing)
    ...
    open_csv_folder_endMon = customtkinter.CTkButton(master=page, text="Open", font=("Roboto", 12), command=__open_file_endMon)
    progressBar_endMon.set(0)
    ...
    trunk_sel.grid(column=1, row=1, padx=10, pady=12, columnspan=3)
    trunk_label.grid(column=0, row=1, padx=10, pady=12, columnspan=3)
...
def __tf_page(root):
    ...
    back_button = customtkinter.CTkButton(master=page, text="Back", font=("Roboto", 12), command=__change_Menu)
    start_button = customtkinter.CTkButton(master=page, text="Start", font=("Roboto", 12), command=__startTFThread)
    ...
    open_csv_folder_TF = customtkinter.CTkButton(master=page, text="Open", font=("Roboto", 12), command=__open_file_tf)
...
def __change_Menu():
    quest.logInfo("Go to __Menu")
    global root
    for widget in root.winfo_children():
        widget.destroy()
    __Menu(root)

def __check_Login_filled():
...
def __startEndMonSing():
    ...
    if config.debug == False:
        ...
        elif trunk.get() not in trunks:
            quest.logInfo("INVALID TRUNK")
            CTkMessagebox(title="Invalid Trunk", message=f"Not a valid trunk name!", icon="cancel")
...
def __startEndMonThread():
    if config.debug == False:
        if endMon_month_start.get() == '':
            ...
            quest.logInfo("STARTING NEW THREAD")
            thread = threading.Thread(target=__startEndMon)
            thread.start()
    else:
        CTkMessagebox(title="DEBUG", message=f"Scripts can't be run in debugging mode!", icon="cancel")
...
def __startTFThread():
    if config.debug == False:
        if tf_month_start.get() == '':
            ...
            quest.logInfo("STARTING NEW THREAD")
            thread = threading.Thread(target=__startTF)
            thread.start()
    else:
        CTkMessagebox(title="DEBUG", message=f"Scripts can't be run in debugging mode!", icon="cancel")
...
def __writeRememberMe():
    with open('./temp/login.tmp', 'w') as read:
        read.write('1' + '\n')
...
def main():
    ...
    root.iconbitmap(__find_by_relative_path(("assets" + os.sep + "hdtecIcon.ico")))
    Login(root)
    root.protocol("WM_DELETE_WINDOW", __on_quit)
    root.mainloop()
    quest.closeLog()
